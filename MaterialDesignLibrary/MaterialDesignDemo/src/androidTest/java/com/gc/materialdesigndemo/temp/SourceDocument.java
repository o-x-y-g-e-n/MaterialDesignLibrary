...
import static com.gc.materialdesigndemo.temp.StringHelpers.abbreviate;
import static com.gc.materialdesigndemo.temp.XMLHelpers.toNodeName;
import static com.gc.materialdesigndemo.temp.XMLHelpers.toSafeString;
...
public class SourceDocument {
    ...
    private static String toXmlNodeName(@Nullable String className) {
        ...
        if (!fixedName.equals(className)) {
            Log.d("hell",String.format("Rewrote class name '%s' to XML node name '%s'", className, fixedName));
        }
        return fixedName;
    }
    ...
    private void serializeView(View view, final int depth) throws IOException {
        ...
        if (depth < MAX_TRAVERSAL_DEPTH) {
            ...
            Log.d("hell1",String.format("Skipping traversal of %s's children, since the current depth " +
        ...
        for (Class<?> streamType : new Class[]{ByteArrayOutputStream.class, FileOutputStream.class}) {
            ...
            try {
                ...
                if (streamType.equals(FileOutputStream.class)) {
                    ...
                    outputStream = AutomationCore.getInstance().getInstrumentation().getContext().openFileOutput(tmpXmlName, Context.MODE_PRIVATE);
                } else {
                ...
                try {
                    ...
                    Log.d("hell3",String.format("The source XML tree has been fetched in %sms using %s",
                ...
                return outputStream instanceof FileOutputStream
                        ? AutomationCore.getInstance().getInstrumentation().getContext().openFileInput(tmpXmlName)
    ...
    private void performCleanup() {
        if (tmpXmlName != null) {
            AutomationCore.getInstance().getInstrumentation().getContext().deleteFile(tmpXmlName);
            tmpXmlName = null;
        }
    }
